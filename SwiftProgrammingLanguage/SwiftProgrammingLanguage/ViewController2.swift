...
/*lzy170920注:
 ...
 存储属性 简单来说，一个存储属性就是存储在特定类或结构体实例里的一个常量或变量。存储属性可以是变量存储属
 ...
 常量结构体的存储属性
 如果创建了一个结构体的实例并将其赋值给一个常量，则无法修改该实例的任何属性，即使有属性被声明为变量
 也不行:
 ...
 第 2 章 Swift 教程 | 157
 延迟属性很有用，当属性的值依赖于在实例的构造过程结束后才会知道影响值的外部因素时，或者当获得属性的
 初始值需要复杂或大量计算时，可以只在需要的时候计算它。
 下面的例子使用了延迟存储属性来避免复杂类中不必要的初始化。例子中定义了 DataImporter 和 DataManage r 两个类，下面是部分代码:
 class DataImporter {
 /*
 ...
 */
 ...
 • Point 封装了一个 (x, y) 的坐标
 • Size 封装了一个 width 和一个 height • Rect 表示一个有原点和尺寸的矩形
 ...
 上述例子中创建了一个名为 square 的 Rect 实例，初始值原点是 (0, 0) ，宽度高度都是 10 。如下图中蓝 色正方形所示。
 ...
 let fourByFiveByTwo = Cuboid(width: 4.0, height: 5.0, depth: 2.0) print("the volume of fourByFiveByTwo is \(fourByFiveByTwo.volume)") // 打印 "the volume of fourByFiveByTwo is 40.0"
 这个例子定义了一个名为   的结构体，表示三维空间的立方体，包含   、 性。结构体还有一个名为   的只读计算属性用来返回立方体的体积。为 义，因为无法确定如何修改   、   和   三者的值来匹配新的 一个只读计算属性来让外部用户直接获取体积是很有用的。
 属性观察器
 和   属 提供 setter 毫无意
 。然而，   提供
 第 2 章 Swift 教程 | 160
 ...
 didSet
 注意
 必须使用   关键字定义计算属性，包括只读计算属性，因为它们的值不是固定的。 关键字只用来声明 常量属性，表示初始化后再也无法修改的值。
 ...
 • 在新的值被设置之前调用
 • 在新的值被设置之后立即调用
 观察器会将新的属性值作为常量参数传入，在   的实现代码中可以为这个参数指定一个名 称，如果不指定则参数仍然可用，这时使用默认名称 表示。
 同样，   观察器会将旧的属性值作为参数传入，可以为该参数命名或者使用默认参数名   。如果 在   方法中再次对该属性赋值，那么新值会覆盖旧的值。
 注意
 父类的属性在子类的构造器中被赋值时，它在父类中的 和   观察器会被调用，随后才会调用
 ...
 oldValue
 willSet
 子类的观察器。在父类初始化方法调用之前，子类给属性赋值时，观察器不会被调用。 有关构造器代理的更多 信息，请参考值类型的构造器代理 (页 0)和类的构造器代理规则 (页 0)。
 下面是一个   和   实际运用的例子，其中定义了一个名为 步行时的总步数。这个类可以跟计步器或其他日常锻炼的统计装置的输入数据配合使用。
 class StepCounter {
