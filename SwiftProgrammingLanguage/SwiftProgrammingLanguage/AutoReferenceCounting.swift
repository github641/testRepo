...
/*lzy170930注:
 ...
 然而在少数情况下，为了能帮助你管理内存，ARC 需要更多 关于代码之间关系的信息。本章描述了这些情况，并 且为你示范怎样才能使 ARC 来管理你的应用程序的所有内存。在 Swift 使用 ARC 与在 Obejctive-C 中使用 ARC 非常类似，具体请参考过渡到 ARC 的发布说明。
 注意
...
class AutoReferenceCounting: UIViewController {
    ...
    override func viewDidLoad() {
       ...
       /*
        ...
        /*
         ...
         接下来的代码片段定义了三个类型为 Person? 的变量，用来按照代码片段中的顺序，为新的 Person 实例建立多个引用。由于这些变量是被定义为可选类型( Person? ，而不是 Person )，它们的值会被自动初始化为 nil ，目 前还不会引用到 Person 类的实例。
         */
        ...
        // 现在你可以创建特定的 Person1 和 Apartment1 实例并将赋值给 john 和 unit4A 变量:
        ...
        /*
         现在，两个关联在一起的实例的引用关系如下图所示:john 和 unit4A弱引用关系.jpg
...
*/
        ...
        /*
         在你关联两个实例后，它们的引用关系如下图所示:Customer 和 CreditCard 无主引用.jpg
        ...
        /*
         ...
         不同，这次一个是类实例，另一个是闭包。
         Swift 提供了一种优雅的方法来解决这个问题，称之为 闭包捕获列表 (closure capture list)。同样的，在学 习如何用闭包捕获列表打破循环强引用之前，先来了解一下这里的循环强引用是如何产生的，这对我们很有帮助。
