...
class Subscripts: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
         ...
         /*下标可以定义在类、结构体和枚举中，是访问集合，列表或序列中元素的快捷方式。可以使用下标的索引，设置和获取值，而不需要再调用对应的存取方法。举例来说，用下标访问一个 Array 实例中的元素可以写作 y[index] ，访问 Dictionary 实例中的元素可以写作 someDictionary[key] 。
        ...
        /*
         ...
         newValue 的类型和下标的返回类型相同。如同计算型属性，可以不指定 setter 的参数( newValue )。
         如果不 指定参数，setter 会提供一个名为 newValue 的默认参数。
...
*/
        ...
        /*
         ...
         例如，Swift 的 Dictionary 类型实现下标用于对其实例中储存的值进行存取操作。为字典设值时，在下标中使用 和字典的键类型相同 的键，并把一个 和字典的值类型相同的值 赋给这个下标:
        ...
        struct Matrix {
            ...
            init(rows: Int, columns: Int) {
                ...
                // swift3.0的语法， grid = Array(count: rows * columns, repeatedValue: 0.0)
                // 这是Swift4.0的语法，将repeatElement放到了前面
        ...
        /*
         ...
         rows * columes 个 Double 类型的值的数组。通过传入数组长度和初始值 0.0 到数组的构造器，将矩阵中每个位置的值初始化 为 0.0 。关于数组的这种构造方法请参考创建一个空数组 (页 0)。
        ...
        /*lzy170926注:
         会触发 get方法中的断言：
         assertion failed: Index out of range: file /Users/alldk/testRepo/SwiftProgrammingLanguage/SwiftProgrammingLanguage/ViewController.swift, line 116
