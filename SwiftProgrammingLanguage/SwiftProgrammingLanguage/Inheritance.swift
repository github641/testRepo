...
class Inheritance: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        /*
         ...
         新的 Bicycle类自动获得  Vehicle 类的所有特性，比如 currentSpeed和  description 属性，还有它的 makeNoise()方法。
         除了它所继承的特性， Bicycle  类还定义了一个默认值为 false  的存储型属性 hasBasket  (属性推断为 Bool)。
         默认情况下，你创建任何新的 Bicycle实例将不会有一个篮子(即 hasBasket  属性默认为false 之后，你可以为特定的Bicycle 实例设置 hasBasket  属性为true   :
         */
        let bicycle = Bicycle()
        bicycle.hasBasket = true
        /*你还可以修改 Bicycle  实例所继承的  currentSpeed 属性，和查询实例所继承的 description属性
         */
        ...
        /*
         Tandem有 Vehicle和Bicycle的所有属性和方法，且增加了一个存储型属性currentNumberOfPassengers，默认值为0
         如果你创建了一个Tandem的实例，你可以使用它所有的新属性和继承的属性，还能查询从Vehicle
         继承来的只读属性:description
         */
        let tandem = Tandem()
        ...
        /*
         ...
         如果要重写某个特性，你需要在重写定义的前面加上override关键字。这么做，你就表明了你是想提供一个重写 版本，而非错误地提供了一个相同的定义。意外的重写行为可能会导致不可预知的错误，任何缺少override关键 字的重写都会在编译时被诊断为错误。
        ...
        // MARK:  ======重写属性的 Getters 和 Setters
        /*
         通过给属性写getter和setter来重写属性。
         重写属性的时候必须把属性的名字和类型都写出来。
         不可以将一个继承来的读写属性重写为一个只读属性。
         如果你在重写属性中提供了 setter，那么你也一定要提供 getter。
         
         你可以提供定制的 getter(或 setter)来重写任意继承来的属性，无论继承来的属性是存储型的还是计算型的属性。子类并不知道继承来的属性是存储型的还是计算型的，它只知道继承来的属性会有一个名字和类型。你在 重写一个属性时，必需将它的名字和类型都写出来。这样才能使编译器去检查你重写的属性是与超类中同名同类 型的属性相匹配的。
         
         你可以将一个继承来的只读属性重写为一个读写属性，只需要在重写版本的属性里提供 getter 和 setter 即可。但是，你不可以将一个继承来的读写属性重写为一个只读属性。
         注意
         如果你在重写属性中提供了 setter，那么你也一定要提供 getter。如果你不想在重写版本中的 getter 里修改继承来的属性值，你可以直接通过 super.someProperty 来返回继承来的值，其中 someProperty 是你要重写的属 性的名字。
        ...
        // MARK: ======重写属性观察器
        /*
         你可以通过重写属性,为一个继承来的属性添加属性观察器。
         你不可以为继承来的常量存储型属性添加属性观察器。
         你不可以为继承来的只读计算型属性添加属性观察器。
         不可以同时提供重写的 setter 和重写的属性观察器。
         
         你可以通过重写属性,为一个继承来的属性添加属性观察器。这样一来，当继承来的属性值发生改变时，你就会被通知到，无论那个属性原本是如何实现的。关于属性观察器的更多内容，请看属性观察器 (页 0)。
        ...
        */
        ...
        /*
         当你设置AutomaticCar的currentSpeed属性，属性的didSet观察器就会自动的设置gear属性，为新的速度选择一个合适的档位。具体来说就是，属性观察器将新的速度值除以 10，然后向下取得最接近的整数值，最后加1
         来得到档位 的值。例如，速度为 35.0时，挡位为 :
         */
        ...
        // MARK: -  防止重写
        /*你可以通过把方法，属性或下标标记为final来防止它们被重写，只需要在声明关键字前加上fianl修饰符即 可(例如: final var， final func， final class func，以及 final subscript)。
         如果你重写了带有final标记的方法，属性或下标，在编译时会报错。在类扩展中的方法，属性或下标也可以在 扩展的定义里标记为 final 的。
         你可以通过在关键字class前添加 final  修饰符( final class  )来将整个类标记为 final 的。这样的类是不可 被继承的，试图继承这样的类会导致编译报错。
         */
